...
`0.43.0-alpha`, state schema `35`, checkpoint format `5`, operation schema `4`,
...
`BuildProposalVisitor`, dump/RVA evidence, staged-collateral safety boundary,
pointer-free module hydration, regressions, and fresh-session live gate.
...
receipt-bound re-archive, strict newest-local discovery, and the remaining live
reload gate.
...
source rule that prevents recurrence.

[Research-export architecture boundary](RESEARCH_EXPORT_BOUNDARY_2026-08-09.md)
records the extraction from the size-capped game entrypoint, exact report-field
parity, corrected limitation inventory, bridge receipt/failure contract, and
the Lua truthiness regression caught by its focused test.

[Witnessed aboard-evidence hardening](WITNESSED_ABOARD_EVIDENCE_HARDENING_2026-08-09.md)
records the short-route alighting race, exact monotonic load witnesses, stale
no-op/retry behavior, evidence-priority FIFO, checkpoint action binding, and
...
replacement consist/service facts, empty legacy-line capacity, and missing
post-session economy evidence while preserving the honest live boundary.
...
per-station train barrier, format-4 convergence projection with exact model passenger state, restart behavior,
...
- [CONSTRUCTION_OWNER_PLAN_REGRESSION_2026-08-31.md](CONSTRUCTION_OWNER_PLAN_REGRESSION_2026-08-31.md) - relay-proven modular-terminal rejection caused by trusting engine-mutated `PlayerOwned` userdata, immutable canonical owner-plan repair, adversarial terminal-family matrix, and the next packaged native gate.
