...
The second item is not yet finished same-world multiplayer. Prototype `0.21`, state schema `21`, checkpoint format `3`, edge proposal schema `5`, and construction proposal schema `7` converge from the same populated save, give pre-existing assets the same canonical ownership, replay bounded construction transactions, reconcile canonical finance, compare direct passenger telemetry, and establish all-peer checkpoints without putting engine-local IDs on the wire. Schema 5 adds named signals/waypoints and preserves them across track replacement. Schema 7 adds portable named construction payloads for depots, ordinary constructions, `ASSET_GROUP`-only assets, observable upgrades, modular station edits, and removal. The ordinary-UI localhost construction matrix now passes for signals/waypoints, depot placement/use, stock modular-station placement/edit/removal, bench placement/removal with rival denial, and lamp/fence placement. Construction uses an all-peer prepare/commit barrier: geometry and named resources must resolve everywhere before either world mutates. Shared-clock v2 uses future simulation-time rendezvous, paused heartbeats, catch-up rounds, and a slowest-peer cap. Assigned canonical trains are held at every station until both copies report the same vehicle/line/stop/leg, then receive one future-time release. A populated exact-build localhost run completed four such releases with zero faults and matching final core/structure/mobility state. Two-computer latency, disconnect recovery, and unsupported command families remain open. See [PROTOTYPE_STATUS.md](PROTOTYPE_STATUS.md), [REMAINING_FROM_BRIEF.md](REMAINING_FROM_BRIEF.md), and [the train/clock synchronization investigation](investigation/TRAIN_CLOCK_RENDEZVOUS_AND_STATION_BARRIERS_2026-08-06.md) for the precise boundary.
...
- A deterministic contested-demand economy (model version 4) with passenger and cargo market kinds: every service attribute — fare, journey time, wait, transfers, comfort, lagged crowding — converts to a generalized cost in cents; demand splits by an integer-exact logit over a pinned exponential table; market share is a persistent stock with carried residuals, while an upward fare shock adopts the lower equilibrium immediately so yesterday's riders cannot be harvested at a new price. Options beyond the 8-theta cutoff receive zero weight, and authored aggregates saturate below Lua 5.1's exact-integer ceiling. Kind weighting is market data — cargo waits at half the passenger weight, pays 1800 seconds per transshipment, values time at 60 cents/hour, and competes against trucking at an $18 outside cost — so one evaluator, one fare latch, and one replay path serve both kinds. Archived model-v2/v3 events retain their legacy paths. Evaluation, revenue, reach, wins, and model value replay digest-identically in Lua and Python.
- Checksummed checkpoint format `3`, immutable digest-chained events, independent Python model replay, and canonical state that excludes machine-local numeric IDs. Checkpoint convergence includes authored state, canonical bindings, structural state, canonical company balances/loans, and authorized per-vehicle station-release rounds. Formats 1 and 2 remain readable for archived evidence.
...
- Canonical line and railway-vehicle operation codecs with strict schemas, host/company authorization, replay/result checking, finance routing, physical consensus, and checkpoint tests. Ordinary vanilla **New Line**, stop add/remove/reorder/terminal updates, and line deletion feed the line codec through typed capture. Stock railway **Buy** pairs the GUI's ordered `vehicle/train/` + `vehicle/waggon/` consist with the pinned native player/depot payload, and stock **Set Line** is captured from the native visitor; both commands remain pre-mutation until host order and replay on every peer. Purchase, assignment, peer visibility, movement, and four station rounds now have two-process evidence. Replace, sell, start/stop, reverse, maintenance, and send-to-depot do not yet all have equivalent transparent vanilla capture.
...
The current strongest live network proof is `runtime/localhost-live/train-station-fresh-clock-20260806-0630`. Two exact Build 35924 processes loaded the same populated save under an immediate post-load pause, waited for fresh clock samples from both peers, then ran its real NOHAB + two-BC4 train through four alternating station releases. Host status ended at one tracked train, four releases, zero pending rounds/faults, final clock skew zero, and converged mobility/lifecycle/route phase. Both worlds finished at core `1fea40f9`, model `98f01295`, structure `e1488bff`, and mobility `6fca8ed2`; audit replay reports 15 convergences, two successful physical proposals, and three successful checkpoint barriers. See [the train/clock synchronization investigation](investigation/TRAIN_CLOCK_RENDEZVOUS_AND_STATION_BARRIERS_2026-08-06.md). The older `populated-network-ownershipfix-20260803` receipt remains the static ownership/mapping baseline.
...
- Match initialization and each successful physical outcome immediately open a checkpoint barrier. Later network intents remain blocked until both peers report the same format-3 convergence key, including canonical company finances and authorized train station rounds, and consume the ordered checkpoint outcome.
...
- Shared-clock rendezvous v2 and the per-station train barrier pass protocol, checkpoint, restart, timeout, adversarial, and populated two-process tests. Four alternating station releases completed with zero skew/faults. Exact-coordinate repair remains deliberately absent; different stop indices fault closed. Long alternating-peer pauses, deliberate slow-peer recovery, disconnect/reconnect, and two-computer latency remain live gates. Native passenger/cargo steering remains open.
...
- `tpf2_mp_1/` — installable prototype 0.21; state schema 21, checkpoint format 3, edge schema 5, and construction schema 7.
