...
- Canonical freight authority behind the loaded-content gate. Each game captures the construction resources it actually loaded, the companion strictly merges them, and all peers attest the exact registry before the host bootstraps canonical live industries. State 29/freight schema 2 persist recipes, input/output stock, production residuals/totals, and exact per-line boarded/delivered cursors. Cargo-only endpoint lines deterministically bind the nearest compatible source output and destination stock within 500 metres; named vanilla or mod cargo capacity is resolved for every assigned consist, and each vehicle keeps its exact capacity. Ordered station releases own queues, loads, completed deliveries, and unit-kilometre revenue. A five-minute boundary atomically validates aggregate reservations, withdraws source stock, deposits destination stock, advances production, records money, and moves both presentation epochs. Standard cargo/line/vehicle/station/manager surfaces show these authored values. Native cargo agents and history remain cosmetic. Missing, changed, ambiguous, overdrawn, or reload-incompatible content, contracts, stocks, and cursors fault closed. The exact vanilla registry remains 16 positive-flow resources and 160 variants at digest `edc7a517`; cargo-positive live play is the remaining proof gate.
- Native hook `0.16.0`, pinned to the exact Windows x64 Build 35924 executable. It validates SHA-256, PE metadata, 17 unique signatures, and 31 entries in the command visitor table; observes the native command pipeline; wraps `api.cmd.sendCommand`; strictly gates tag-15 `BuildProposal`; and gates 31 consequential/autonomy tags before mutation. Tags 17-24 now close the native Create/Remove/Develop Town, SetTownInfo, town-cargo, town/industry-connect, manual-industry-development, and building-closure command surface. Ordered town development, town information, and industry freezing consume exact one-shot tags 19, 20, and 23; the other five have no gameplay authorization path. The hook also converts suppressed vanilla pause/speed clicks into ordered shared-clock requests; captures typed CreateLine/DeleteLine/UpdateLine plus SetName/SetColor payloads from the ordinary line manager as optimistic origin pass-throughs; and captures pointer-free BuyVehicle, SetLine, reverse, start/stop, maintenance, depart, send-to-depot, SellVehicle, ReplaceVehicle, and manual-departure identities before mutation. Config-bearing buy/replace commands are correlated with the bounded stock GUI consist. Tag 12 uses a versioned envelope that retains the complete bounded, duplicate-free stock selection for canonical batch sale. Build 35924 asserts if the line visitors reject, so any later rejection of an origin-applied line operation faults the session closed and requests the ordered pause.
...
- A second all-peer checkpoint barrier after match start, every successful physical action, and every unanimously rejected no-mutation proposal. Later commands stay blocked until both peers attest the same core, structure, and finances. **Prepare & Save Restore Point** orders a shared pause/quiescence/checkpoint boundary; only after both the local runtime and companion identify the same READY boundary and core does each peer issue an automatically named native save. The watcher waits for a stable save triplet, hashes it, files an ordered peer receipt, archives it, and reports it in the launcher. Current restore-plan v4 binds both load-bearing files (`.sav` and `.sav.lua`) plus the source session's exact agent/town-development profile; launch adopts that policy, re-hashes both files, and rejects explicit conflicts. Existing v2 main-save and v3 profile-only plans remain readable with explicit legacy semantics. Exact names tolerate only the small READY-poll race; arbitrary prefixed manual saves still have to follow the watcher observation. Each peer also captures one bounded local first-fault bundle before process-liveness cleanup, including its bridge, copied audit, exact session logs, native status, and source/install fingerprint. The automatic SaveGame request and watcher race are offline-proven but still need a two-live-process proof; automatic geometry repair and rollback are not claimed.
...
- A one-button coordinated restore boundary: pause/quiescence preparation,
  distinct peer save receipts, v2/v3/v4 restore-plan verification, peer-local
...
- A prepare rejection is non-fatal because no native world has changed. A post-commit native rejection is also recoverable only when every peer fails, emits no outputs or finance delta, agrees on result/core/error, and retains the exact prepare-barrier core; it receives its own checkpoint before play continues. Mixed results, residue, digest mismatch, or the default 45-second physical/checkpoint timeout still fault the session closed. In-place native geometry repair is not implemented. The game now requests one peer-specific native save only while the shared pause/quiescence/checkpoint boundary remains READY and the current core matches it; the watcher hashes the stable result and files the receipt. That exact-boundary automatic capture is offline-proven and still needs a two-process live run. Verified manual launcher relaunch now exists; automatic no-selection rollback remains open.
...
The plan names the exact convergence boundary and new session ID. Host sessions also create `latest-recovery-archive.json` once both peer-specific READY-gated saves have ordered receipts. The automatic capture path is not considered live-proven until both real game processes produce and reload those saves; it is restart authority, never an unsafe geometry importer.
