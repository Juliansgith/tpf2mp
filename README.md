...
- A deterministic contested-demand economy (model version 2): every service attribute — fare, journey time, wait, transfers, comfort, lagged crowding — converts to a generalized cost in cents; demand splits by an integer-exact logit over a pinned exponential table; market share is a persistent stock that glides toward equilibrium with asymmetric up/down rates and a carried residual, so entrants climb from zero and fare milking bleeds share faster than it recovers; the outside option is the conservation residual, so better service grows ridership. Evaluation, revenue, reach, wins, and model value replay digest-identically in Lua and Python.
