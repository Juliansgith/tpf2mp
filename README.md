...
This runs the full suite, rebuilds the native DLL/injector, creates a one-file `tpf2mp.exe`, writes SHA-256 metadata for every packaged file, creates `dist\TPF2MP-0.24.0-alpha.zip`, and performs a temporary install/verify/uninstall round trip.
...
The current strongest ordinary-line proof is `runtime/localhost-live/train-prompt-barrier-state22-20260806-105918`. Two exact Build 35924 processes loaded the same populated save and ran its real NOHAB + two-BC4 train through four prompt station rounds: zero scheduled and four unscheduled releases, zero pending rounds, zero faults, 1.86-second average and 2.38-second maximum wall-clock latency. Both worlds finished at core `fba1630d`, model `98f01295`, structure `15189409`, and mobility `8e5d90e6`; the audit reports 15/15 commit convergences, 2/0/0 physical proposals, and 3/0/0 checkpoint barriers. The scheduled-service baseline remains `train-scheduled-state22-20260806-1010`: three scheduled release commits, zero faults, and matching final state. A later human run completed 39 scheduled rounds and proved speed-3 recovery after deliberately delaying Player 2, but also exposed the now-removed artificial fallback dwell. See [the schedule/barrier-load investigation](investigation/STATION_SCHEDULE_INTEGRATION_AND_BARRIER_LOAD_2026-08-06.md).
...
- `tpf2_mp_1/` — installable prototype 0.24; state schema 24, checkpoint format 4, passenger-presentation schema 1, edge schema 5, and construction schema 7.
