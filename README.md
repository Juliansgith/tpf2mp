...
The second item is not yet finished same-world multiplayer. Prototype `0.30`, state schema `27`, checkpoint format `4`, passenger-presentation schema `2`, edge proposal schema `5`, and construction proposal schema `7` now cover canonical construction, atomic topology-plus-building demolition, lines, stock train purchase/assignment, station-leg synchronization, exact model passenger queues/loads and completed-trip revenue, save-owned economy difficulty, canonical model-town growth, authored physical town development, receipt-bound coordinated restore, authoritative projection into the standard game UI, and exact all-peer agreement on the loaded industry recipes before match start. Exact two-process runs converge the canonical/model/structural domains, while cargo stocks and delivery, unsupported command families, and the first true two-computer usability gate remain open. See [PROTOTYPE_STATUS.md](PROTOTYPE_STATUS.md), [REMAINING_FROM_BRIEF.md](REMAINING_FROM_BRIEF.md), and [the investigation index](investigation/README.md) for the precise evidence boundary.
...
- A loaded-industry content gate. Each game captures the construction resources it actually loaded into immutable content-addressed artifacts; the companion strictly merges them, omits non-flow helpers, and exposes a session/peer-bound sidecar. The game revalidates that sidecar, binds live industry roots to evaluated recipes, and requires one ordered attestation from each peer before initialization. The exact vanilla registry is 16 positive-flow resources and 160 variants at digest `edc7a517`, with zero ambiguity. Missing, changed, or incompatible content faults closed. This establishes recipe authority only; canonical cargo stocks, queues, loads, deliveries, and cargo-positive play remain open.
...
This runs the full suite, rebuilds the native DLL/injector, creates a one-file `tpf2mp.exe`, writes SHA-256 metadata for every packaged file, creates `dist\TPF2MP-0.30.0-alpha.zip`, and performs a temporary install/verify/uninstall round trip.
...
- `tpf2_mp_1/` — installable prototype 0.30; state schema 27, economy model 7, checkpoint format 4, passenger-presentation schema 2, edge schema 5, and construction schema 7.
