...
// Prologue only: push chain, `lea rbp, [rsp-0x15B0]`, and `mov eax, 0x16B0`
// (the frame size handed to __chkstk). The pattern deliberately stops before
// the `call __chkstk` that follows, because that call's rel32 displacement
// targets RVA 0x02BF3B60 and changes whenever the linker moves either side,
// while these 24 bytes are rsp-relative and immediate operands only. The cut
// is still unique in the Build 35924 image (one hit, at kMakeBuildProposalRva).
inline constexpr std::array<std::uint8_t, 24> kMakeBuildProposal{
