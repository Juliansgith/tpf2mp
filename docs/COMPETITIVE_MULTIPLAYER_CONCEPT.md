...
**Vehicle drift is bounded at service boundaries.** Replicas need not integrate identical mid-leg physics, but they share one negotiated clock and each canonical vehicle rendezvouses at the authored boundaries that change competitive load: every conservative passenger stop, the two endpoints of an urban road/tram route, or an exact freight source/sink. A vehicle may be slightly ahead between those anchors; it may not silently gain an extra completed leg because one player opened a pause menu or one computer ran faster.

**The visible world must support the competitive truth.** Native passengers and freight do not have to match the economic model agent-for-agent, but the stock game surfaces must tell the same story. A service winning most of a market cannot be presented as empty while the loser appears full. Authoritative loads, queues, deliveries, revenue, and score should agree closely enough that players can read the contest from the game world; native agents may remain cosmetic when no safe target-addressed mutation exists.
...
The host runs a deterministic economic model and an authoritative world-event log alongside the native game. Player intentions go to the host; the host validates and commits them, assigns stable canonical identities to created objects, and broadcasts ordered events whose references each machine translates to its own local engine IDs. Autonomous systems such as town and industry development are frozen or driven by the host through the same event stream. The model owns demand pools, route attractiveness, fares, revenue, contracts, industry allocation, station queues, and vehicle loads, using host-read, canonicalised world facts rather than independently sampled client state. Native vehicle motion remains the operational layer and rendezvouses at service boundaries; stock UI projections expose the exact authored passenger/freight state while untargetable native agents remain cosmetic. This avoids requiring every internal simulation detail to run in perfect lockstep without tolerating structural or scoring divergence between the two worlds.
