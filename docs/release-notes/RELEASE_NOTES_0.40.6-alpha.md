...
This release adds ordered handling for rapid stock line edits and a narrowly
proven in-place recovery path for non-mutating proposal timeouts. It advances
the save state to schema 32; checkpoint format 5, the relay wire protocol, and
native hook 0.19.0 remain unchanged.
...
- Both players must install `0.40.6-alpha`; mixed versions are unsupported.
