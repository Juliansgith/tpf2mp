...
Vanilla roads, tracks, stations, station modules, depots, signals, waypoints, assets, bulldozing, and supported upgrades have live bidirectional capture/replay evidence with company authorization. Vehicle purchase, line assignment, peer visibility, and movement passed a human two-instance run. Shared-clock v2 turns pause, resume, and speed changes into future rendezvous barriers, projects fresh peer heartbeats to host time, caps speed to the slowest healthy peer, and faults failed corrections. A per-canonical-vehicle station barrier holds every replica at the same stop until all peers report the same round, then orders one future release. Ordinary and registered services use the same prompt physical guard; model headway remains an economy input and does not impose a second native timetable. This bounds accumulated service divergence at operationally meaningful points without requiring identical mid-leg physics.
