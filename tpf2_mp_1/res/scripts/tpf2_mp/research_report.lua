...
local KNOWN_LIMITS = {
  "BuildProposal uses a hybrid pre-mutation capture: the exact native factory proposal is correlated through CommandList::Add and the suppressed tag-15 visitor, while the bounded GUI preview supplies semantic fields not independently decoded from the native layout. One generated registry describes all 37 command tags and mechanically gates native visitor behavior plus Lua operation/proposal codec replay. Unsupported categories remain fail-closed.",
  "Proposal schema 6 canonically serializes road/track changes plus named signal/waypoint edge objects, including retained objects across edge replacement, with quoted cost and no machine-local IDs. Schema 8 adds stock rail-station placement and bounded generic named .con/.module payloads for depots, ordinary constructions, ASSET_DEFAULT roots, upgrades, modular station edits, and removal. Both paths use repository names, strict ownership, preflight and physical consensus. Opaque/script callbacks and ambiguous dependency migration fail closed; every peer still requires an identical pinned mod pack.",
  "Construction uses all-peer prepare before native mutation, then two-peer physical completion consensus, ordered success/fault controls, a bounded timeout, and fail-closed dependency gating. PREPARE identity discovery is read-only; accepted unique topology rebinding is rollback-journaled with ownership and custody. Cheap binding fingerprints and periodic full structural probes detect different drift classes. A readiness rejection is non-fatal because neither world changed; in-place native geometry rollback is deliberately not claimed.",
  "Shared-clock v2 projects staggered peer heartbeats to one host time, orders future-time pause/speed rendezvous, corrects bounded overshoot, emits paused heartbeats, and adaptively caps the effective speed from engine/backlog health. Populated localhost and physical two-computer relay use are live-proven; multi-hour load certification remains.",
  ...
  "A READY-gated automatic native save request, stable-file watcher, ordered peer receipts, paired archive, restore-plan handoff, and clean-save continuation are implemented. Automatic repair of already-diverged native geometry and host migration remain open.",
}
...
function M.new(env)
  ...
  local function build()
    ...
    for _, field in ipairs({
      ...
      "performance",
      "nativeFingerprint", "nativeFingerprintScheduler",
    }) do
