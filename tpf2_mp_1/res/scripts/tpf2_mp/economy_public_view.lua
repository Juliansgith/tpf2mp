...
function M.build(state, activeCompanyCid)
  ...
  for _, lineCid in ipairs(util.sortedKeys(economyState.services or {})) do
    ...
    result.services[lineCid] = {
      ...
      stationAccessSchema = metadata.stationAccessSchema,
      stationAccessSource = metadata.stationAccessSource,
      stationAccessEligible = metadata.stationAccessEligible,
      endpointAccessReady = util.deepCopy(metadata.endpointAccessReady or {}),
      endpointReachableBuildings = util.deepCopy(
        metadata.endpointReachableBuildings or {}),
      endpointTownBuildings = util.deepCopy(metadata.endpointTownBuildings or {}),
      allocated = latest.allocated or 0,
      requested = latest.requested or latest.allocated or 0,
      capacityOverflow = latest.capacityOverflow or 0,
      ...
      loadPpm = latest.lagLoadPpm or service.lagLoadPpm or 0,
