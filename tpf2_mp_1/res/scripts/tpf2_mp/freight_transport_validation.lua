...
function M.migrate(state, saved)
  ...
  if schema == 1 then return {
    transportCursors = {}, totalTransported = {}, totalDelivered = {}, lastTransport = nil,
  } end
  if schema ~= 2 or type(saved.transportCursors) ~= "table" then
  ...
  for _, lineCid in ipairs(util.sortedKeys(saved.transportCursors)) do
    ...
    if type(cursor.contractDigest) ~= "string" or #cursor.contractDigest ~= 8
        ...
        or not matchingOutput(source, cursor.cargoType)
        or not matchingInput(destination, cursor.destinationStockIndex, cursor.cargoType) then
  ...
  local savedTransported, transportedError = counterMap(
    saved.totalTransported, "saved total transported")
  if not savedTransported then return nil, transportedError end
  local savedDelivered, deliveredError = counterMap(
    saved.totalDelivered, "saved total delivered")
  if not savedDelivered then return nil, deliveredError end
  if not sameCounters(transported, savedTransported)
      or not sameCounters(delivered, savedDelivered) then
  ...
  if last ~= nil then
    ...
    for field in pairs(last) do
      if field ~= "lines" and field ~= "boarded" and field ~= "delivered" then
    ...
    if last.lines > #util.sortedKeys(cursors) then
      return nil, "saved last freight transport line count is impossible"
    end
    for cargo, count in pairs(boarded) do
      if count > (transported[cargo] or 0) then return nil, "saved last boarded exceeds totals" end
    end
    for cargo, count in pairs(arrived) do
      if count > (delivered[cargo] or 0) then return nil, "saved last delivered exceeds totals" end
    end
    last = { lines = last.lines, boarded = boarded, delivered = arrived }
  end
  return { transportCursors = cursors, totalTransported = transported,
    totalDelivered = delivered, lastTransport = last }
end
