...
Last updated: 2026-08-08 after prototype `0.29.0-alpha`, state schema `26`,
...
| Save/load | State migrates through schema 26, including canonical shared-save ownership, accounts, bindings, proposal/checkpoint consensus, shared-clock rendezvous, train release rounds, backwards-compatible legacy slot state, exact passenger presentation, save-owned difficulty, canonical model towns/growth residuals, authored physical town-development progress, and receipt-bound restore state. Pre-difficulty saves migrate to Normal. Byte-pinned load is automated after title entry. Fresh bootstrap verifies manager-visible ownership; a prepared two-peer boundary, distinct save receipts, schema-2 restore plan, reload, mandatory fresh checkpoint, and resumed train service pass live. | Live save/reload during an active held/releasing/loaded round and after several model-town growth ticks, more than two pre-existing native companies, long-running sessions, and removal of the remaining manual stock Save-dialog step. |
...
| Frozen autonomy | Town/industry freeze and readback are implemented and used in authority validation. Competitive passenger demand now has its own deterministic canonical town-size/growth layer, so native crowd capacity is never market truth. | Long unpaused no-player-change drift trace; live-prove model growth under the default skeleton policy; then finish host-authored native town presentation and industry/cargo events one subsystem at a time. |
...
now exist; live-prove the standard-UI projection and tune its boarding cadence. Keep
unowned autonomous subsystems frozen, add industry change only as ordered events
with physical postconditions, then give cargo the same canonical queue/load
ledger after a cargo-positive source run. Native people may remain bounded
scenery unless a future exact-build operation can address a specific target.
