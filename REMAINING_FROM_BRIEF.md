...
| Contested passenger demand | Model version 2: generalized cost in cents (fare/time/wait/transfers/comfort/lagged crowding), integer-exact logit shares over a pinned exp table, share-as-stock with asymmetric glide and exact conservation through the outside-option residual, induced demand, and cross-language digest-identical replay. Invariant tests cover conservation, entrant ramp, milking defense, stall-free convergence, and crowding stability. | Hot-seat playtests to tune VOT/theta/alpha per market, bind services to real corridors, decide transfer behavior, and balance match pacing. |
