...
| Authoritative history/replay | Ordered commits/controls, digest-chained events, checkpoint reports, and independent Python replay pass. The deterministic randomized long trace now interleaves settlements, market re-registration, and autonomy toggles for 1,024 post-checkpoint events. Each peer's existing recovery watcher captures one bounded local first-fault bundle before game-liveness cleanup, including copied bridge/audit, exact session log tails, native status, and source/install fingerprint. Functional fixtures prove capture even after the game PID is gone. | Human-live-prove both per-peer bundle paths after a deliberate fault; add automatic rotation/retention and full replay projections for any future operation schema. |
