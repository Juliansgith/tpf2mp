...
Last updated: 2026-08-07 after prototype `0.24.0-alpha`, state schema `24`,
checkpoint format `4`, passenger-presentation schema `1`, edge proposal schema `5`, construction proposal schema `7`, populated two-process convergence,
...
State 24 additionally passes receipt-bound two-peer restore and three rounds of
...
| Railway vehicles | Canonical operation representation, authorization, replay/result, finance, consensus, and checkpoints exist. Hook 0.13 captures stock BuyVehicle player/depot scalars before mutation and correlates them with the GUI's exact loco+waggon consist; SetLine has typed native capture. An exact NOHAB + two BC4 purchase succeeds in a disposable Build 35924 process and through the ordinary UI on two independent processes with matching physical/checkpoint results and isolated finances. The station barrier holds each copy until both peers report the same vehicle/line/stop/round. State 24 gives registered competitive services canonical departure slots and advances their exact passenger loads at that same ordered boundary; ordinary lines use the barrier with a short guard and no invented timetable. Scheduled and unscheduled exact-build runs converge all four digests with zero faults, the 50-train authority stress passes, and a human speed-3 run recovered after substantially delaying Player 2. | Live multi-train throughput/signaling, a line registered to a real market with passenger-HUD readback, two-computer latency, and disconnect/reconnect recovery. Add transparent start/stop, reverse, maintenance, send-to-depot, replacement and sale capture, depot/failure constraints, and safe paused relaunch recovery. Road vehicles, ships, and aircraft follow afterward. |
...
| Passenger/cargo presentation | Passenger schema 1 deterministically converts settled allocation into endpoint queues and per-train loads at ordered station releases. State 24/checkpoint 4 digest the full canonical route, counts, endpoints, and release cursor; the selection-aware TPF2MP HUD displays exact synchronized train/station/line numbers. Direct ECS telemetry still measures native scenery. | Human two-process display proof and pacing polish. The stock native passenger glyph remains cosmetic because tag 36 cannot address a target; cargo needs a corresponding authoritative ledger and cargo-positive proof. |
...
now exist; live-prove the passenger HUD and tune its boarding cadence. Keep
