...
| Vehicles | Canonical operation representation, authorization, replay/result, finance, consensus, and checkpoints exist. The vehicle config now accepts any safe `vehicle/*.mdl` resource, so vanilla/mod-added rail, road, tram, ship, and air models share one portable codec and native cost path. Hook 0.13 captures BuyVehicle player/depot scalars before mutation and correlates them with the GUI config; SetLine has typed native capture. An exact NOHAB + two BC4 purchase succeeds in a disposable Build 35924 process and through the ordinary UI on two independent processes with matching physical/checkpoint results and isolated finances. The station barrier holds each copy until both peers report the same vehicle/line/stop/round, then uses a short guarded release. Exact passenger loads advance at the same ordered boundary. | Railway remains the only carrier with human two-process purchase/movement proof. Live multi-train throughput/signaling, settled market/HUD readback, two-computer latency, and disconnect/reconnect recovery remain. Add transparent start/stop, reverse, maintenance, send-to-depot, replacement and sale capture, depot/failure constraints, safe paused relaunch recovery, then live-prove road/tram/ship/air. |
