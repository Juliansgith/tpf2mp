...
Last updated: 2026-08-06 after prototype `0.22.0-alpha`, state schema `22`,
...
| Save/load | State migrates through schema 22, including canonical shared-save ownership, accounts, bindings, proposal/checkpoint consensus, shared-clock rendezvous state, canonical train release rounds and departure-slot reservations, and stable construction/asset/edge-object manifests. Byte-pinned native starting-save load is automated after title entry. Fresh bootstrap now projects logically owned manager-visible entities onto each peer's native company representatives and fails before checkpoint if readback disagrees; exact reload plus human Line/Vehicle Manager visibility pass. | Live save/reload during a held/releasing train round, active failure recovery, more than two pre-existing native companies, and long-running sessions. |
