"""Compare TPF2MP's fast terrain height refinement with ORIGINAL executable code, natively.
...
overlapping regions) must be identical. The fast path is
TPF2MP_TerrainFastTestRefine in tpf2mp_hook_build35924.dll. No game process or
game file is touched.

Adapted from silver2127's tpf2-bigmap tools/test_terrain_refine.py
...
are dropped (TPF2MP pins the Steam build only).
  ...
  python tests/native_terrain_fast/refine_proof.py [--exe EXE] [--dll DLL]
"""
import ctypes as C
import random
import struct
...
def main():
    ...
    fast = harness.export(dll, 'TPF2MP_TerrainFastTestRefine')
    ...
    for failure in failures:
        ...
        def patcher(context, rva, ptr, size):
            events.append(('patch', rva, size))       # refine patches nothing
            return 1
        host = harness.Host(None, getbase, verify, hook, patcher)
        mask, error = install(host, 0 if failure == 'disabled' else harness.FEATURE_REFINE)
        assert mask == (harness.STATUS_REFINE if failure == 'none' else 0), (failure, mask, error)
    ...
    print(f'PASS: installer verifies the 20-byte prologue (5 whole instructions, no RIP/branches) and '
          f'{sum(n for _, n in REGIONS)} bytes of body/product/constants; an empty feature mask, each '
          f'mismatch and a hook failure refuse')
