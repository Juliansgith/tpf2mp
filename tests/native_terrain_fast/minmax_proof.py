"""Compare TPF2MP's terrain min/max fast path with ORIGINAL executable code, natively.
...
Adapted from silver2127's tpf2-bigmap tools/test_terrain_minmax.py
...
are dropped (TPF2MP pins the Steam build only), and the block-copy benchmark
calls the exported entry point instead of a raw detour address (TPF2MP exports
no BlockCopyAddress/SetOriginal pair).

  python tests/native_terrain_fast/minmax_proof.py [--exe EXE] [--dll DLL]
"""
import ctypes as C
import math
import random
...
def build_bench(nargs=10):
    """MS-x64 bench(ctx): ctx[0] = callee, ctx[1..nargs] = its arguments,
    ctx[nargs+1] = iterations. Calls it in a native loop, so a per-call time is
    the callee's own cost and not ctypes marshalling. The 0x58 frame keeps
    rsp % 16 == 0 and holds up to seven stack arguments (eleven in total), which
    is what TPF2MP_TerrainFastTestBlockCopy needs: the original pointer first,
    then the stock function's ten."""
    assert 4 <= nargs <= 11
    code = bytearray(bytes.fromhex('5355565741544155415641574881ec58000000'))
    code += bytes.fromhex('4889cb')                 # mov rbx, rcx
    code += load('r12', 'rbx', 8 * (nargs + 1))
    loop = len(code)
    for reg, slot in (('rcx', 1), ('rdx', 2), ('r8', 3), ('r9', 4)):
        code += load(reg, 'rbx', 8 * slot)
    for slot in range(5, nargs + 1):
...
def main():
    ...
    if max(a + i.size for a, i in func.items()) != FUNC_END:
        harness.fail(f'unexpected build: {exe} does not decode Build 35924 bytes at {FUNC:#x}')
    ...
    # ------------------------------------------------ patch and harnesses
    scan_addr = harness.export(dll, 'TPF2MP_TerrainFastTestMinMaxScanAddress')
    scan_addr.restype = C.c_size_t
    scan_fn = scan_addr()
    make_patch = harness.export(dll, 'TPF2MP_TerrainFastTestMinMaxPatch')
    ...
    scan = harness.export(dll, 'TPF2MP_TerrainFastTestMinMaxScan')
    ...
    fast_copy = harness.export(dll, 'TPF2MP_TerrainFastTestBlockCopy')
    ...
    # ---------------------------------------------------------- installer
    # TPF2MP's seam passes the context first, takes no steal count (MinHook
    # measures the prologue itself) and has no log service or GOG build, so the
    # bigmap steal/log/GOG checks are gone: "disabled" is an empty feature mask,
    # refusals are read from the error buffer, and the two halves are asserted
    # through their own status bits instead of one boolean.
    install = harness.installer(dll)
    sites = [(LOOP, LOOP_END - LOOP), (EPI, EPI_END - EPI), (COPY, COPY_END - COPY)]
    expected_mask = {'none': harness.STATUS_MINMAX_SCAN | harness.STATUS_BLOCK_COPY,
                     'patch': harness.STATUS_BLOCK_COPY, 'hook': harness.STATUS_MINMAX_SCAN}
    for failure in ('none', 'disabled', 'verify0', 'verify1', 'verify2', 'patch', 'hook', 'both'):
        events, errors = [], []
        ...
        def hook(context, target, detour, original):
            events.append(('hook', target - IB, COPY_STEAL))
            if target != IB + COPY or not detour:
                errors.append('hook')
            original[0] = target
            return failure not in ('hook', 'both')

        host = harness.Host(None, getbase, verify, hook, patcher)
        mask, error = install(host, 0 if failure == 'disabled' else harness.FEATURE_MINMAX)
        ...
        assert mask == expected_mask.get(failure, 0), (failure, mask, error)
        refusals = [part for part in error.split('; ') if part]   # AppendError joins with "; "
        assert len(refusals) == {'none': 0, 'disabled': 0, 'both': 2}.get(failure, 1), (failure, error)
        if failure == 'disabled':
            assert events == [] and error == '' and not mask
        elif failure.startswith('verify'):
            assert events == full[:int(failure[-1]) + 1] and not mask, (failure, events)
        else:
            assert events == full, (failure, events)
    print('PASS: installer preflights all three byte ranges before patching; refuses when the feature mask is empty '
          'or on any mismatch; one recorded refusal per failed half; partial install only when one half fails')
    ...
    # TPF2MP exports no detour address / SetOriginal pair, so the fast side runs
    # through TPF2MP_TerrainFastTestBlockCopy(original, ...): one extra register
    # argument and one extra call is the only overhead the detour itself lacks.
    fast_copy_addr = C.cast(fast_copy, C.c_void_p).value
    benches = {count: BENCHFN(executable(build_bench(count))) for count in (10, 11)}
    bench_ctx = (C.c_uint64 * 16)()

    def bench_call(addr, call_args, iters):
        bench_ctx[0] = addr
        for i, v in enumerate(call_args):
            bench_ctx[1 + i] = v & 0xffffffffffffffff
        bench_ctx[len(call_args) + 1] = iters
        t = time.perf_counter()
        benches[len(call_args)](C.addressof(bench_ctx))
    ...
    warm = []
    for addr, call_args in ((stock_copy_addr, args), (fast_copy_addr, (stock_copy_addr,) + args)):
        dst.fill(0)
        bench_call(addr, call_args, 50)
        assert np.array_equal(dst, src)          # the benchmark really copied
        warm.append(min(bench_call(addr, call_args, 2000) for _ in range(5)))
