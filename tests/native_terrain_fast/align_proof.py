"""Compare TPF2MP's fast terrain alignment with ORIGINAL executable code, natively.
...
The fast path is TPF2MP_TerrainFastTestAlign in tpf2mp_hook_build35924.dll,
which calls the SAME mapped rasterizer. No game process or installed file is
touched.

Adapted from silver2127's tpf2-bigmap tools/test_terrain_align_fast.py
...
are dropped (TPF2MP pins the Steam build only).

  python tests/native_terrain_fast/align_proof.py [--exe EXE] [--dll DLL]
"""
import ctypes as C
import random
import struct
...
def map_image(pe):
    ...
    if base != IMAGE:
        harness.fail(f'preferred image base {IMAGE:#x} is not free in this process ({base})')
...
def main():
    ...
    if pe.get_data(FUNC, len(PROLOGUE)) != PROLOGUE:
        harness.fail(f'unexpected build: {exe} has no Build 35924 prologue at {FUNC:#x}')
    stock = FN(IMAGE + FUNC)
    dll = harness.load_dll(dll_path)
    # The export resolves the stock rasteriser/vtable addresses from `base` on
    # every call (bigmap cached them in a global); that is a handful of adds and
    # the only difference from what the installed detour runs.
    fast = harness.export(dll, 'TPF2MP_TerrainFastTestAlign')
    ...
    #    measures the prologue itself) and has no GOG build, so those bigmap
    #    checks and cases are gone; "disabled" becomes an empty feature mask.
    ...
    failures = ['none', 'disabled', 'hook'] + [('verify', i) for i in range(len(expected_verify))]
    for failure in failures:
        ...
        def patcher(context, rva, ptr, size):
            events.append(('patch', rva, size))       # align patches nothing
            return 1
        host = harness.Host(None, getbase, verify, hook, patcher)
        mask, error = install(host, 0 if failure == 'disabled' else harness.FEATURE_ALIGN)
        assert mask == (harness.STATUS_ALIGN if failure == 'none' else 0), (failure, mask, error)
        refusals = [part for part in error.split('; ') if part]   # AppendError joins with "; "
        assert len(refusals) == (0 if failure in ('none', 'disabled') else 1), (failure, error)
    ...
    print(f'PASS: installer verifies the 21-byte prologue (9 whole instructions, no RIP/branches), '
          ...
          f'vtables at the running base; an empty feature mask, each mismatch and a hook failure refuse')
