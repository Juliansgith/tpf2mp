...
local constructionConnectionReplay = require "tpf2_mp/construction_connection_replay"
...
local constructionExactTopology = require "tpf2_mp/construction_exact_topology"
...
test("canonical JSON and cross-language checksum", function()
  ...
  equal(decoded.a, "x")
  equal(decoded.b, 2)
  equal(json.encode(json.decode([[{"array":[],"object":{}}]])),
    [[{"array":[],"object":{}}]],
    "JSON decode erased the wire distinction between an empty array and object")

  -- Python-authored control envelopes are verified after Lua decoding.  This
  -- checksum is generated by companion.tpf2mp.protocol.sign and specifically
  -- protects the empty-array recovery checkpoint that once wedged both native
  -- inbox cursors before a later vehicle.buy operation.
  local pythonSigned = json.decode([[{"checksum":"29383965","kind":"control","origin_peer":"player1","payload":{"action":{"items":[],"metadata":{},"type":"test.array"}},"protocol":1,"seq":20,"session":"array-roundtrip","tick":0}]])
  truthy(bridge.verify(pythonSigned),
    "Lua rejected a valid Python-signed envelope containing an empty array")
end)
...
test("proposal codec canonicalises the measured smallest modular passenger station", function()
  ...
  -- Snapping a through station into an existing track can replace the old
  -- carrier edge and use a canonical boundary at both ends. This is a normal
  -- stock GUI placement, not an unsupported station edit.
  local insertedRaw = smallestStationProposal(700)
  local insertedNodes = insertedRaw.proposal.addedNodes
  local insertedEdges = insertedRaw.proposal.addedSegments
  table.remove(insertedNodes, #insertedNodes)
  table.remove(insertedNodes, 1)
  insertedEdges[1].comp.node0 = 7001
  insertedEdges[#insertedEdges].comp.node1 = 7004
  insertedRaw.proposal.edgesToRemove = { 7002 }
  local inserted, insertedError = proposalCodec.normalise(insertedRaw, "company:1", {
    resolveCanonical = function(kind, localId)
      local values = {
        node = { [7001] = "node:pre:station-west", [7004] = "node:pre:station-east" },
        edge = { [7002] = "edge:pre:station-through" },
      }
      return values[kind] and values[kind][localId] or nil
    end,
    resourceName = function(kind, index) return kind .. "/" .. index .. ".lua" end,
  })
  truthy(inserted, insertedError)
  equal(#inserted.nodes, 11)
  equal(inserted.remove.edges[1], "edge:pre:station-through")
  equal(inserted.edges[1].node0.cid, "node:pre:station-west")
  equal(inserted.edges[#inserted.edges].node1.cid, "node:pre:station-east")
  truthy(proposalCodec.validatePortable(inserted))
  truthy(constructionReplayState.requiresAtomic({ transaction = inserted }, proposalCodec),
    "station insertion lost its exact topology replay requirement")
...
test("exact construction replay safely replaces both generated snap endpoints", function()
  local function node(entity, x)
    return { entity = entity, comp = { position = { x = x, y = 0, z = 0 } } }
  end
  local function edge(entity, node0, node1)
    return {
      ...
      trackEdge = { trackType = 1, catenary = false },
      playerOwned = { player = 100 },
    }
  end
  local command = { proposal = {
    proposal = {
      addedNodes = { node(-1, 10), node(-2, 0), node(-3, 20) },
      addedSegments = { edge(-4, -2, -1), edge(-5, -1, -3) },
      edgeObjectsToAdd = {},
    },
    toAdd = { { playerEntity = 999 } },
  } }
  local exact = {
    nativePlayerId = 100,
    nodes = { node(-11, 10) },
    edges = { edge(-12, 7001, -11), edge(-13, -11, 7004) },
    objects = {}, edgeOwners = { 100, 100 },
    playerOwnedFactory = function() return {} end,
  }
  local result, resultError = constructionExactTopology.applyProcessed(command, exact)
  truthy(result, resultError)
  equal(result.discarded.nodes, 2)
  equal(#command.proposal.proposal.addedNodes, 1)
  equal(command.proposal.proposal.addedSegments[1].comp.node0, 7001)
  equal(command.proposal.proposal.addedSegments[2].comp.node1, 7004)
  equal(command.proposal.proposal.addedSegments[1].trackEdge.catenary, false,
    "exact replay discarded a false track catenary flag")
  equal(command.proposal.toAdd[1].playerEntity, 100)
end)

test("exact construction replay preserves false street feature flags", function()
  local function node(entity, x)
    return { entity = entity, comp = { position = { x = x, y = 0, z = 0 } } }
  end
  local function edge(entity, node0, node1, streetType, hasBus, tramTrackType)
    return {
      ...
      streetEdge = {
        streetType = streetType, hasBus = hasBus,
  ...
  local command = { proposal = {
    proposal = {
      addedNodes = { node(-1, 0), node(-2, 10) },
      addedSegments = { edge(-3, -1, -2, 4, false, 0) },
      edgeObjectsToAdd = {},
    },
    toAdd = { { playerEntity = 999 } },
  } }
  local exact = {
    nativePlayerId = 100,
    nodes = { node(-11, 0), node(-12, 10) },
    edges = { edge(-13, -11, -12, 4, false, 0) },
    objects = {}, edgeOwners = { 100 },
    playerOwnedFactory = function() return {} end,
  }
  local result, resultError = constructionExactTopology.applyProcessed(command, exact)
  truthy(result, resultError)
  equal(command.proposal.proposal.addedSegments[1].streetEdge.hasBus, false,
    "exact replay discarded a false bus-lane flag")
  equal(command.proposal.proposal.addedSegments[1].streetEdge.tramTrackType, 0)
end)
...
test("proposal codec carries portable depots, arbitrary constructions, upgrades, and removals", function()
  ...
  -- The helper cannot receive an explicit existing carrier endpoint. Connected
  -- road, tram and rail depots therefore require an atomic
  -- helper-root/topology-repair pair.
  ...
  truthy(constructionConnectionReplay.connectedDepotCarrier(
      snappedTramDepot, snappedTramDepot.constructions[1]) == "street"
  ...
  local snappedRailDepot = util.deepCopy(snappedRoadDepot)
  snappedRailDepot.constructions[1].fileName = "depot/train_depot_era_a.con"
  snappedRailDepot.constructions[1].params = {
    catenary = 0, trackType = 0, paramX = 0, paramY = 0, seed = 19, year = 1900,
  }
  snappedRailDepot.edges[1].carrier = "track"
  snappedRailDepot.edges[1].resource = {
    index = 0, name = "standard.lua",
  }
  snappedRailDepot.edges[1].catenary = false
  snappedRailDepot.edges[1].bus = nil
  snappedRailDepot.edges[1].tramTrackType = nil
  snappedRailDepot.edges[1].node1 = { cid = "node:pre:rail-depot-approach" }
  snappedRailDepot.digest = proposalCodec.digest(snappedRailDepot)
  snappedRailDepot.transactionId = "proposal:" .. snappedRailDepot.digest
  truthy(proposalCodec.validatePortable(snappedRailDepot),
    "snapped rail-depot fixture is not portable")
  local snappedRailRecord = { transaction = snappedRailDepot }
  truthy(constructionReplayState.isConnectedDepot(
      snappedRailDepot, snappedRailDepot.constructions[1])
      and constructionConnectionReplay.connectedDepotCarrier(
        snappedRailDepot, snappedRailDepot.constructions[1]) == "track"
      and not constructionReplayState.isExact(snappedRailRecord, proposalCodec)
      and depotConnectionRepair.isRepairable(snappedRailRecord)
      and constructionReplayState.requiresAtomic(snappedRailRecord, proposalCodec),
    "connected rail depot did not enter carrier-generic helper repair")
  ...
  truthy(constructionConnectionReplay.hasExistingCarrierEndpoint(
  ...
  -- Live relay regression mp-094022e94f4ae9c3: Build 35924 can silently snap
  -- a generated depot access edge to a pre-existing canonical track node.
  -- Carrier-generic helper repair must admit that exact endpoint relation.
  ...
  truthy(snappedOk, snappedError)
  truthy(depotConnectionRepair.isRepairable({ transaction = snappedDepot }),
    "snapped rail depot did not enter helper-root topology repair")
...
test("portable topology resolution rejects a reused live local id before proposal binding", function()
  local previousApi, previousGame = api, game
  local nodes = {
    [10] = { position = { x = 12.3, y = 45.6, z = 7.8 } },
  }
  game = { interface = {
  ...
  local registry = canonical.newState()
  local cid = assert(world.bindExisting(registry, 10, "node"))
  local expectedTopology = registry.byCanonical[cid].metadata.topologyFingerprint
  truthy(expectedTopology, "origin binding did not retain portable topology")

  -- Simulate a loaded save/native replacement reusing 10 for another node
  -- while the intended node receives a different machine-local allocation.
  nodes[10] = { position = { x = -80, y = 20, z = 0 } }
  nodes[20] = { position = { x = 12.3, y = 45.6, z = 7.8 } }
  local inspected, inspectError, inspectDetails = world.findPreExistingLocal(
    registry, cid, "node", { worldState = { logicalOwners = {}, pinnedCustody = {} } })
  equal(inspected, 20, tostring(inspectError) .. "; expected="
    .. tostring(expectedTopology) .. "; old="
    .. tostring(world.topologyFingerprint(10, "node")) .. "; new="
    .. tostring(world.topologyFingerprint(20, "node")))
  truthy(inspectDetails and inspectDetails.source == "topology-fallback"
      and inspectDetails.rebindNeeded and not inspectDetails.mutated,
    "proposal PREPARE did not discover the unique replacement read-only")
  equal(canonical.resolveLocal(registry, cid), 10,
    "read-only proposal inspection mutated the stale binding")

  local worldState = {
    logicalOwners = { ["10"] = "company:1" },
    pinnedCustody = { ["10"] = { cid = cid } },
  }
  local rebound, rebindError = world.resolvePreExisting(registry, cid, "node", {
    owner = "company:1", worldState = worldState,
  })
  api, game = previousApi, previousGame
  equal(rebound, 20, rebindError)
  equal(canonical.resolveLocal(registry, cid), 20)
  equal(worldState.logicalOwners["20"], "company:1")
  truthy(worldState.pinnedCustody["20"],
    "accepted proposal binding did not move canonical custody")
end)
...
test("line transport-mode reading distinguishes passenger, cargo, and indexed mixed groups", function()
  ...
  local lineReading = require("tpf2_mp/world_line_reading").new({
    ...
    resolveCanonical = function(_, kind, id)
      return kind .. ":canonical:" .. tostring(id)
    end,
    fingerprint = function(id, kind)
      return kind .. ":fingerprint:" .. tostring(id)
    end,
  })
  ...
  local descriptor = lineReading.nativeDescriptor(100, {})
  equal(descriptor.name, "entity-100")
  equal(descriptor.stops[1].stationGroupCid, "station_group:canonical:901")
  equal(descriptor.stops[2].stationGroupCid, "station_group:canonical:902")
  equal(descriptor.stops[1].station, 0)
  fakeApi.type.ComponentType.TRANSPORT_VEHICLE = "TRANSPORT_VEHICLE"
  components.TRANSPORT_VEHICLE = { [777] = { line = 100 }, [778] = { line = -1 } }
  equal(lineReading.vehicleLine(777, {}), "line:canonical:100")
  equal(lineReading.vehicleLine(778, {}), nil)
  equal(lineReading.vehicleLine(779, {}), nil)
end)
...
test("native inbox rewinds instead of draining ordered work after validation failure", function()
  local priorConfigure = rawget(_G, "tpf2mp_native_bridge_configure")
  local priorEmit = rawget(_G, "tpf2mp_native_bridge_emit")
  local priorTake = rawget(_G, "tpf2mp_native_bridge_take")
  local configuredIn, configureCount = {}, 0
  local badReturned, goodReturned = false, false
  local good = {
    protocol = 1, session = "native-rewind", seq = 1, kind = "commit",
    origin_peer = "player1", origin_local_seq = 7, tick = 4,
    payload = { action = { type = "native.recovered" } },
  }
  good.checksum = hash.value(good)

  tpf2mp_native_bridge_configure = function(_, _, nextIn)
    configureCount = configureCount + 1
    configuredIn[#configuredIn + 1] = nextIn
    return "A1|1"
  end
  tpf2mp_native_bridge_emit = function() return "A1" end
  tpf2mp_native_bridge_take = function()
    if configureCount == 1 and not badReturned then
      -- Simulate destructive native delivery of the durable record with a
      -- corrupt checksum.  A transport reconfigure must occur before the
      -- corrected/reloaded record may be returned.
      badReturned = true
      return "I1|1|{\"checksum\":\"bad\",\"protocol\":1,\"session\":\"native-rewind\",\"seq\":1}"
    end
    if configureCount >= 2 and not goodReturned then
      goodReturned = true
      return "I1|1|" .. json.encode(good) .. "\n"
    end
    return nil
  end

  local state = bridge.newState({
    protocol = 1, root = tempRoot .. "/native-rewind",
    peerId = "player2", sessionId = "native-rewind",
  })
  local rejected = bridge.poll(state, 4)
  equal(#rejected, 0)
  equal(state.nextInSeq, 1, "invalid native record advanced the Lua cursor")
  equal(state.nativeTransport.active, false,
    "invalid native record left the destructive transport active")
  equal(state.nativeTransport.lastRejectedInSeq, 1)
  equal(state.nativeTransport.rewinds, 1)
  truthy(tostring(state.lastError):match("checksum mismatch"), state.lastError)

  local recovered = bridge.poll(state, 4)
  equal(configureCount, 2, "native inbox was not reconfigured after rejection")
  equal(configuredIn[1], "1")
  equal(configuredIn[2], "1", "native inbox did not rewind to the rejected cursor")
  equal(#recovered, 1)
  equal(recovered[1].payload.action.type, "native.recovered")
  equal(state.nextInSeq, 2)
  equal(state.lastError, nil)
